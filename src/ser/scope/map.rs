...
impl MapScope {
    ...
    /// Rename a key of an existing entry according to the given case convention.
    ///
    /// Same effect as [`rename_key`](Self::rename_key), but the new key is generated by
    /// applying case conversion to the existing key.
    ///
    /// The existing key must be of string type ([`Value::Str`](crate::Value::Str)), however the passed in selector can match
    /// the key by index. If the existing key happens to be of another type, no renaming is performed.
    ///
    /// Returns `self` to allow chaining calls.
    pub fn rename_key_case(
        &mut self,
        key: impl Into<MapKeySelector>,
        case: impl Into<Case>,
    ) -> &mut Self {
        self.actions
            .push(MapEntryAction::RenameCase(key.into(), case.into()));
        self
    }

    /// Rename all keys all entries according to the given case convention.
    ///
    /// This applies [`rename_key_case`](Self::rename_key_case), to all string type
    /// keys ([`Value::Str`](crate::Value::Str)) in the map that are not renamed or replaced explicitly.
    /// Keys of other types are not affected.
    ///
    /// Returns `self` to allow chaining calls.
    pub fn rename_all_fields_case(&mut self, case: impl Into<Case>) -> &mut Self {
        self.actions
            .push(MapEntryAction::RenameAllCase(case.into()));
        self
    }
}
